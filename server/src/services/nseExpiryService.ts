 ...
 */
export const OFFICIAL_MARKET_HOLIDAYS: MarketHolidayInfo[] = [
...
export class NseExpiryService {
  ...
  public static getHolidaysList(): MarketHolidayInfo[] {
  ...
  private static nthDayOfMonths(dayOfMonth: number, count: number, from: Date, exchange: 'NSE' | 'BSE' | 'MCX' = 'MCX'): string[] {
  ...
  private static lastTradingDayOfMonths(count: number, from: Date, exchange: 'NSE' | 'BSE' | 'MCX' = 'MCX'): string[] {
    const expiries: string[] = [];
    for (let m = 0; expiries.length < count && m < count + 12; m++) {
   ...
   */
  public static precedingTradingDay(date: Date, exchange: 'NSE' | 'BSE' | 'MCX' = 'NSE'): Date {
