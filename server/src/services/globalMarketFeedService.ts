...
export class GlobalMarketFeedService {
  ...
  private generateBaselineGlobalContext(): GlobalMarketContextData {
  ...
  private computeGlobalRiskMode(ind: GlobalMarketContextData['indicators']): GlobalRiskMode {
    ...
    // FII inflows
    if (ind.fiiNetBuyCr > 0) riskScore += 1;

    if (riskScore >= 5) return 'RISK_ON';
    if (riskScore >= 3) return 'NEUTRAL';
    if (riskScore >= 1) return 'RISK_OFF';
