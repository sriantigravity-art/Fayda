...
export interface McxOfflineQuote {
  symbol: string;
  name: string;
  ...
  unit: string;          // e.g. "₹/10g", "₹/kg", "₹/bbl"
  source: 'MCX_WEBSITE' | 'IBJA_CACHED';
  settlementDate: string; // Last settlement date
}

export interface McxIcomdexIndex {
  name: string;          // e.g. "MCX iCOMDEX Composite"
  value: number;
  change: number;
  pctChange: number;
}

export interface McxOfflineData {
  commodities: McxOfflineQuote[];
  icomdex: McxIcomdexIndex[];
  marketStatus: 'CLOSED' | 'HOLIDAY' | 'PRE_OPEN';
  lastUpdated: string;
  closingDate: string;   // Date of the last session data
}
...
class McxOfflineService {
  private cache: { data: McxOfflineData; ts: number } | null = null;
   ...
   */
  private async scrapeFromMcxWebsite(): Promise<Partial<McxOfflineData>> {
    const headers = {
      'User-Agent': UA,
    ...
    const icomdex: McxIcomdexIndex[] = [];
    const commodities: McxOfflineQuote[] = [];
    ...
    for (const url of jsonEndpoints) {
      try {
        ...
        const indices: any[] = data?.indices || data?.Indices || data?.data?.indices || [];
        ...
        const contracts: any[] = data?.contracts || data?.Data || data?.data || (Array.isArray(data) ? data : []);
        ...
        if (icomdex.length > 0 || commodities.length > 0) break;
      } catch { /* try next */ }
    }
   ...
   */
  private async fetchIbjaFallback(): Promise<McxOfflineQuote[]> {
    try {
  ...
  private parseIbjaRows(html: string, tabId: string): Array<{ date: string; gold999: number; silver999: number }> {
  ...
  public static getMcxStatus(): { isOpen: boolean; status: 'OPEN' | 'CLOSED' | 'HOLIDAY' | 'PRE_OPEN' } {
    const now = new Date();
    const utc = now.getTime() + (now.getTimezoneOffset() * 60000);
    const ist = new Date(utc + (3600000 * 5.5));
    const day = ist.getDay(); // 0=Sun, 6=Sat

    if (day === 0 || day === 6) return { isOpen: false, status: 'HOLIDAY' };

    const min = ist.getHours() * 60 + ist.getMinutes();
    if (min >= 9 * 60 - 15 && min < 9 * 60) return { isOpen: false, status: 'PRE_OPEN' };
    if (min >= 9 * 60 && min < 23 * 60 + 30) return { isOpen: true, status: 'OPEN' };
    return { isOpen: false, status: 'CLOSED' };
  }

  // ─── Public API ─────────────────────────────────────────────────────────────

  public async getOfflineData(): Promise<McxOfflineData> {
    ...
    const data: McxOfflineData = {
      commodities,
      icomdex,
      marketStatus: status as McxOfflineData['marketStatus'],
      lastUpdated: now,
