...
const MARKET_HOLIDAYS_SET = new Set<string>([
...
export const isIndianTradingDay = (date: Date): boolean => {
  ...
  if (MARKET_HOLIDAYS_SET.has(yyyyMmDd)) return false;
  return true;
};
 ...
 */
export const getNextTradingDay9AmIST = (fromDate = new Date()): Date => {
...
export class FyersService {
   ...
   */
  public async executeRenewalWithRetries(attempt = 1, maxAttempts = 3): Promise<{ success: boolean; message: string }> {
   ...
   */
  public async refreshAccessToken(pinOverride?: string): Promise<{ success: boolean; message: string; userName?: string }> {
   ...
   */
  public async savePin(pin: string): Promise<{ success: boolean; message: string; config?: PublicFyersConfig; userName?: string }> {
