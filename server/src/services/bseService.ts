...
export interface BseOptionChainResult {
  symbol: IndexSymbol;
  spotPrice: number;
  spotChange: number;
...
export class BseService {
  ...
  private bseCookies: string = '';
  private bseCookieTs: number = 0;

  // ── Cookie warm-up ───────────────────────────────────────────────────────────

  private async ensureBseCookies(): Promise<string> {
    if (this.bseCookies && Date.now() - this.bseCookieTs < 5 * 60 * 1000) return this.bseCookies;
    try {
  ...
  public async fetchOptionChain(
    symbol: 'SENSEX' | 'BANKEX',
    expiry?: string
  ): Promise<BseOptionChainResult | null> {
  ...
  private async fetchFromBseApi(
    symbol: string, scripCode: string, expiry?: string
  ): Promise<BseOptionChainResult | null> {
  ...
  private async fetchFromBsePublicJson(
    symbol: string, scripCode: string, expiry?: string
  ): Promise<BseOptionChainResult | null> {
    const expiryParam = expiry ? encodeURIComponent(expiry) : '';
  ...
  private async fetchFromBseBhavcopy(
  ...
  private async parseBseJson(
    symbol: string, json: any, requestedExpiry?: string
  ): Promise<BseOptionChainResult | null> {
    try {
      const rows: any[] = json?.Table || json?.optionchain || json?.Data || json?.data ||
      ...
      if (!spotPrice) {
        const entry = globalIndicesService.getIndices().find(i =>
          (symbol === 'SENSEX' && i.name?.toLowerCase().includes('sensex')) ||
      ...
      for (const row of rows) {
        ...
        if (selectedExpiry && rowExp && rowExp !== selectedExpiry) continue;
        ...
        if (cOI > 0 || +(row.CE_LTP || row.CallLTP || 0) > 0) {
          e.callOI = cOI; e.callLtp = +(row.CE_LTP || row.CallLTP || row.callLastTradedPrice || 0);
        ...
        if (pOI > 0 || +(row.PE_LTP || row.PutLTP || 0) > 0) {
          e.putOI = pOI; e.putLtp = +(row.PE_LTP || row.PutLTP || row.putLastTradedPrice || 0);
        ...
        if (optType === 'CE' || optType === 'C' || optType === 'CALL') {
          e.callOI = +(row.OpenInterest || row.OI || 0); e.callLtp = +(row.LTP || row.LastPrice || 0);
          e.callVolume = +(row.Volume || row.TotalVolume || 0); e.callOIChangeTotal = +(row.OIChange || row.ChangeInOI || 0);
          totalCallOI += e.callOI;
        } else if (optType === 'PE' || optType === 'P' || optType === 'PUT') {
          e.putOI = +(row.OpenInterest || row.OI || 0); e.putLtp = +(row.LTP || row.LastPrice || 0);
          e.putVolume = +(row.Volume || row.TotalVolume || 0); e.putOIChangeTotal = +(row.OIChange || row.ChangeInOI || 0);
          totalPutOI += e.putOI;
        }
      }
  ...
  private parseBseCsv(symbol: string, csv: string, requestedExpiry?: string): BseOptionChainResult | null {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (iScripCode >= 0 && cells[iScripCode] !== scripCode) continue;
        ...
        if (optType === 'CE' || optType === 'CA' || optType === 'C') {
          e.callOI = oi; e.callLtp = ltp; e.callVolume = vol; e.callOIChangeTotal = oiChg;
          totalCallOI += oi;
        } else if (optType === 'PE' || optType === 'PA' || optType === 'P') {
          e.putOI = oi; e.putLtp = ltp; e.putVolume = vol; e.putOIChangeTotal = oiChg;
          totalPutOI += oi;
        }
      }
