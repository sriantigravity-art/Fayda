...
let activeSymbol: string = 'NIFTY';
const clientActiveSymbols: Map<WebSocket, string> = new Map();
let fastLaneTimer: NodeJS.Timeout | null = null;
let batchQuotesTimer: NodeJS.Timeout | null = null;
let bgPollTimer: NodeJS.Timeout | null = null;
...
const pollBatchQuotes = async () => {
  if (isBatchQuotesBusy) return;
  const effective = brokerManager.getEffectiveLiveBroker();
  if (currentDataSource !== 'FYERS_LIVE' && effective !== 'FYERS') return;
...
const stopAllPolling = () => {
  if (fastLaneTimer) clearInterval(fastLaneTimer);
  if (batchQuotesTimer) clearInterval(batchQuotesTimer);
