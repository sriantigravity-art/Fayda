...
export interface MultiLegStrategySetup {
  ...
  // Payoff Metrics (Per Unit / Lot)
  netDebitCreditPts: number;      // In index points (+ for credit, - for debit)
  netDebitCreditRupees: number;   // In ₹ for 1 lot
  maxProfitPts: number | string;  // Points or 'UNLIMITED'
  maxProfitRupees: number | string;
  maxLossPts: number | string;    // Points or 'UNLIMITED'
  maxLossRupees: number | string;
  riskReward: string;             // e.g. '1:2.4' or 'Asymmetric (Defined Risk)'
  ...
  // Margin & Capital
  estimatedMarginRupees: number;  // With exchange multi-leg hedge benefit
  marginSavingsPct: number;       // ~60% to 75% savings vs naked selling

  // Tactical Playbook & Rules from Fayda Option Strategies
...
export interface SyntheticArbitrageItem {
  ...
  putPrice: number;
  spotPrice: number;
  syntheticPrice: number; // C - P + Strike
  deviationPts: number;   // Deviation from Spot
...
export class FaydaMultiLegEngine {
   ...
   */
  public static evaluateMultiLegStrategies(
    symbol: IndexSymbol,
    spotPrice: number,
    ...
    recommendedStrategy: MultiLegStrategySetup;
    allStrategies: MultiLegStrategySetup[];
    syntheticArbitrage: SyntheticArbitrageItem[];
  } {
    const allStrategies: MultiLegStrategySetup[] = [];
    ...
    // =========================================================================
    const syntheticArbitrage: SyntheticArbitrageItem[] = [];
    ...
    strikesSorted.slice(Math.max(0, atmIndex - 3), Math.min(strikesSorted.length, atmIndex + 4)).forEach(s => {
      ...
      let arbType: 'REVERSAL_ARB' | 'CONVERSION_ARB' | 'FAIR_VALUED' = 'FAIR_VALUED';
