...
interface HistoricalMinuteEntry {
  timestamp: number;
  strikes: Map<number, RawStrikeSnapshot>;
  pcr: number;
  atmPcr: number;
}
...
export class OIEngine {
  ...
  public processSnapshot(
    ...
    strikesRaw: RawStrikeSnapshot[],
    strikeStep: number,
    lotSize: number,
    defaultRange: number = 200,
    dataSource: DataSourceMode = 'FYERS_LIVE',
    expiryDates: string[] = [],
    ...
    for (const raw of strikesRaw) {
      ...
      const callSurge = calculateSurgeScore(
        symbol,
        callOIChange1m,
        avgCallOiChange1m,
        raw.callVolume,
      ...
      const putSurge = calculateSurgeScore(
        symbol,
        putOIChange1m,
        avgPutOiChange1m,
        raw.putVolume,
    ...
    const resistanceLevels: KeyLevel[] = callStrikesSorted.slice(0, 3).map((s, idx) => ({
      levelName: `R${idx + 1}`,
    ...
    const supportLevels: KeyLevel[] = putStrikesSorted.slice(0, 3).map((s, idx) => ({
      levelName: `S${idx + 1}`,
      strikePrice: s.strikePrice,
      oi: s.putOI,
    ...
    const straddleRange: StraddleRangeData = {
      atmStrike,
      atmCallLtp,
      atmPutLtp,
      combinedPremium,
    ...
    let sentiment: PcrData['sentiment'] = 'NEUTRAL';
    if (atmPlusMinus5Pcr >= 1.35) sentiment = 'EXTREMELY_BULLISH';
    else if (atmPlusMinus5Pcr >= 1.10) sentiment = 'BULLISH';
    else if (atmPlusMinus5Pcr <= 0.65) sentiment = 'EXTREMELY_BEARISH';
    else if (atmPlusMinus5Pcr <= 0.88) sentiment = 'BEARISH';
    ...
    const currentStrikeMap = new Map<number, RawStrikeSnapshot>();
    ...
    const heroZeroSignals = gammaEngine.evaluateHeroZeroSignals(
      symbol,
      spotPrice,
      atmStrike,
      strikesData,
    ...
    const indexState: MarketIndexState = {
      ...
      unifiedTipsPackage: (() => {
        ...
        const mappedCarryTrades: UnifiedSmartTip[] = carriedTrades.map(c => {
          ...
          let status: UnifiedSmartTip['status'] = 'CARRIED_FORWARD';
          let actionabilityStatus: UnifiedSmartTip['actionabilityStatus'] = 'IN_ENTRY_ZONE';
          let lifecycleDirective: UnifiedSmartTip['lifecycleDirective'] = 'CARRY_FORWARD_CONTINUE';
          ...
          return {
            id: c.id,
            symbol: c.symbol as IndexSymbol,
            ...
            optionType: c.optionType as any,
