...
import { IvStatus, LiquidityRating } from '../types.js';

export interface StrikeGreeks {
  callTheta: number;       // ₹ decay per day (negative value)
  putTheta: number;        // ₹ decay per day (negative value)
  callThetaPerHour: number;// ₹ decay per trading hour (6.4 hrs / day)
  putThetaPerHour: number; // ₹ decay per trading hour
  callIv: number;          // Exact Call Implied Volatility %
  callIvStatus: IvStatus;  // Cheap, Fair, or Expensive
  putIv: number;           // Exact Put Implied Volatility %
  putIvStatus: IvStatus;
  iv: number;              // Composite Strike IV %
  thetaIntensity: 'EXTREME' | 'HIGH' | 'MODERATE' | 'LOW';
}

export class GreekEngine {
  public static cumulativeDistribution(x: number): number {
  ...
  public static standardNormalDensity(x: number): number {
   ...
   */
  public static solveImpliedVolatility(
    spot: number,
    strike: number,
    daysToExpiry: number,
    ltp: number,
   ...
   */
  public static getIvStatus(ivPct: number): IvStatus {
    if (ivPct < 12.5) return 'CHEAP';
    if (ivPct > 18.0) return 'EXPENSIVE_CRUSH_RISK';
    return 'FAIR';
  }
   ...
   */
  public static evaluateLiquidity(volume: number, oi: number, ltp: number): {
  ...
  public static calculateGreeks(
    ...
    let thetaIntensity: StrikeGreeks['thetaIntensity'] = 'LOW';
