...
export class DhanService {
     ...
     */
    normalizeDhanOptionChain(symbol, data, selectedExpiry) {
        if (!data)
            return null;
        ...
        if (ocData.oc && typeof ocData.oc === 'object') {
            for (const [strikeStr, strikeObj] of Object.entries(ocData.oc)) {
     ...
     */
    async getNearestExpiry(symbol, mapping) {
        try {
    ...
    async fetchBatchQuotes(symbols) {
        if (!this.config.isConnected || this.config.hasDataApi === false)
            return {};
        const quotes = {};
        const idxScrips = [];
