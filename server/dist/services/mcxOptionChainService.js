...
export class McxOptionChainService {
    ...
    async ensureMcxCookies() {
        if (this.mcxCookies && Date.now() - this.mcxCookieTs < 5 * 60 * 1000)
    ...
    async fetchOptionChain(symbol, expiry) {
        ...
        if (result && result.strikes.length > 0) {
            ...
            console.log(`[MCX-OI] ${symbol}: ${result.strikes.length} strikes, spot ₹${result.spotPrice.toLocaleString('en-IN')} ` +
    ...
    async fetchFromMcxApi(symbol, expiry) {
    ...
    // ── MCX Response Parser ──────────────────────────────────────────────────────
    async parseMcxResponse(symbol, json, requestedExpiry) {
        try {
            ...
            for (const row of rows) {
                ...
                if (selectedExpiry && rowExpiry && rowExpiry !== selectedExpiry)
                    continue;
                ...
                const optType = (row.OptionType || row.optionType || row.CP_Flag || row.OptionFlag ||
                    row.option_type || row.Type || '').toUpperCase().trim();
