...
class SignalLedgerService {
    ...
    static sanitizeTradeTime(timeStr, isCommodity = false, fallbackTime = '03:30:00 PM IST') {
        if (!timeStr)
            return undefined;
            ...
            if (fs.existsSync(this.dataFilePath)) {
                ...
                if (Array.isArray(list)) {
                    ...
                    list.forEach(c => {
                        ...
                        if (c.callGivenTime)
                            c.callGivenTime = SignalLedgerService.sanitizeTradeTime(c.callGivenTime, isCommodity, '02:15:00 PM IST');
                        if (c.entryPriceTimeFormatted)
                            c.entryPriceTimeFormatted = SignalLedgerService.sanitizeTradeTime(c.entryPriceTimeFormatted, isCommodity, '02:15:00 PM IST');
                        if (c.targetHitTime)
                            c.targetHitTime = SignalLedgerService.sanitizeTradeTime(c.targetHitTime, isCommodity, '03:20:00 PM IST');
                        if (c.target1HitTimeFormatted)
                            c.target1HitTimeFormatted = SignalLedgerService.sanitizeTradeTime(c.target1HitTimeFormatted, isCommodity, '03:20:00 PM IST');
                        if (c.target2HitTimeFormatted)
                            c.target2HitTimeFormatted = SignalLedgerService.sanitizeTradeTime(c.target2HitTimeFormatted, isCommodity, '03:25:00 PM IST');
                        if (c.stoplossTime)
                            c.stoplossTime = SignalLedgerService.sanitizeTradeTime(c.stoplossTime, isCommodity, '03:15:00 PM IST');
                        if (c.stoplossHitTime)
                            c.stoplossHitTime = SignalLedgerService.sanitizeTradeTime(c.stoplossHitTime, isCommodity, '03:15:00 PM IST');
                        if (c.halfProfitBookTime)
                    ...
                    dateGroups.forEach((items, dateStr) => {
                        ...
                        const sorted = Array.from(seenContract.values()).sort((a, b) => {
                            ...
                            if (isACarry !== isBCarry)
                                return isBCarry - isACarry;
    ...
    applyAdminAction(id, action, exitPrice, adminNotes) {
        ...
        if (exitPrice !== undefined && exitPrice > 0) {
            const isSell = Boolean(call.action?.startsWith('SELL') || call.category === 'OPTIONS_SELL' || call.tradingRole === 'SELLER');
     ...
     */
    getCarriedForwardTrades(symbol) {
        ...
        const carryTrades = lastDayTrades.filter(c => c.status === 'BTST' ||
    ...
    recordOrUpdateMilestone(data) {
        ...
        if (call) {
            ...
            if (data.callGivenTimeFormatted)
                call.callGivenTime = SignalLedgerService.sanitizeTradeTime(data.callGivenTimeFormatted, isCommodity, '02:15:00 PM IST') || data.callGivenTimeFormatted;
            if (data.entryPriceTimeFormatted)
                call.entryPriceTimeFormatted = SignalLedgerService.sanitizeTradeTime(data.entryPriceTimeFormatted, isCommodity, '02:15:00 PM IST');
            if (data.target1HitTimeFormatted)
                call.target1HitTimeFormatted = SignalLedgerService.sanitizeTradeTime(data.target1HitTimeFormatted, isCommodity, '03:20:00 PM IST');
        ...
        const filteredSignals = allDayCalls.filter(c => {
            ...
            if (statusFilter !== 'ALL') {
                ...
                if (statusFilter === 'LOSS' && c.status !== 'STOPLOSS_HIT')
                    return false;
                if (statusFilter === 'NEAR_TARGET' && c.status !== 'NEAR_TARGET' && (c.nearTargetPct ?? 0) < 80)
                    return false;
