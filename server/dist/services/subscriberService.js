...
class SubscriberService {
    ...
    async login(emailOrMobile, password) {
        ...
        if (!found)
            return { success: false, error: 'No account found with this email, username, or mobile number.' };
        if (!found.isActive)
    ...
    issueToken(sub) {
        const payload = { subscriberId: sub.id, role: sub.role, email: sub.email };
    ...
    getOptedIn(channel, planFilter) {
        return Array.from(this.subscribers.values())
            .filter(s => {
            if (!s.isActive)
                return false;
            if (planFilter && planFilter.length > 0 && !planFilter.includes(s.plan))
                return false;
            if (channel === 'EMAIL')
                return s.emailOptIn;
            if (channel === 'WHATSAPP')
                return s.whatsappOptIn;
            if (channel === 'SMS')
                return s.smsOptIn;
            return true;
        })
