export class NtmClusterEngine {
     ...
     */
    classifyMarketRegime(oiChangePct, priceChange, fallbackDirection = 0) {
     ...
     */
    computeCluster(symbol, spotPrice, strikeStep, strikes, spotChange = 0) {
        const today = this.getTodayDateStr();
        const atmStrike = Math.round(spotPrice / strikeStep) * strikeStep;
        // Filter ATM ± 3 strikes (7 strikes total: -3, -2, -1, ATM, +1, +2, +3)
        const targetStrikePrices = [];
        ...
        targetStrikePrices.forEach(({ strike: strikePrice, dist }) => {
            ...
            if (callEval.regime === 'Long Buildup' || callEval.regime === 'Short Covering')
                bullishSignals += 1;
            else if (callEval.regime === 'Short Buildup' || callEval.regime === 'Long Unwinding')
                bearishSignals += 1;
            if (putEval.regime === 'Short Buildup' || putEval.regime === 'Long Unwinding')
                bullishSignals += 1;
            else if (putEval.regime === 'Long Buildup' || putEval.regime === 'Short Covering')
                bearishSignals += 1;
            if (callEval.regime === 'Neutral' && putEval.regime === 'Neutral')
                neutralSignals += 1;
        ...
        if (netBullishScorePct >= 65)
            dominantRegime = 'Long Buildup';
        else if (netBearishScorePct >= 65)
            dominantRegime = 'Short Buildup';
        else if (bullishSignals > bearishSignals)
            dominantRegime = 'Short Covering';
        else if (bearishSignals > bullishSignals)
            dominantRegime = 'Long Unwinding';
        // Consensus Signal
