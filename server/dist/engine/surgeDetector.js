...
function calculateSurgeScore(symbol, oiChange1m, avgOiChange1m, volume, avgVolume, ltpPctChange, pcr1mChange, strike, atmStrike) {
    ...
    if (symbol === 'NIFTY' && absOiChange > 250000)
        oiVelocityScore = Math.max(oiVelocityScore, 85);
    if (symbol === 'NIFTY' && absOiChange > 400000)
        oiVelocityScore = 100;
    if (symbol === 'BANKNIFTY' && absOiChange > 150000)
        oiVelocityScore = Math.max(oiVelocityScore, 85);
    if (symbol === 'BANKNIFTY' && absOiChange > 300000)
        oiVelocityScore = 100;
    if ((symbol === 'SENSEX' || symbol === 'BANKEX') && absOiChange > 50000)
        oiVelocityScore = Math.max(oiVelocityScore, 85);
    if ((symbol === 'SENSEX' || symbol === 'BANKEX') && absOiChange > 100000)
        oiVelocityScore = 100;
    ...
    // 5. ATM Proximity (10%)
    let step = 50;
    if (symbol === 'BANKNIFTY' || symbol === 'SENSEX' || symbol === 'BANKEX' || symbol === 'NIFTYNXT50')
        step = 100;
        ...
        step = 50;
    else if (symbol === 'NATURALGAS' || symbol === 'COPPER')
        step = 5;
    else if (symbol === 'ZINC' || symbol === 'MIDCPNIFTY')
        step = 25;
    ...
    // Composite Score
    const totalScore = Math.min(100, Math.max(0, Math.round((oiVelocityScore * 0.40) +
        (volumeScore * 0.20) +
        (premiumScore * 0.20) +
        (pcrScore * 0.10) +
