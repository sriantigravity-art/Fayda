...
export class ConfluenceEngine {
     ...
     */
    static evaluate10IndicatorConfluence(symbol, action, spotPrice, atmStrike, strikes, pcr, maxPain, tech, patternBreakout, cprData, indiaVixVal) {
     ...
     */
    static getMarketSession(symbol, date = new Date()) {
    ...
    static calculateProfitBoxAndAdvice(params) {
        const { status, pnlPoints, pnlPct, pnlRupees, currentLtp, t1Pct, isExpiryDay, isCommodity, nextExpiryDate, entryPrice, target1Price, target2Price, stoplossPrice, confluenceScore, marketRegime } = params;
        const baseEntry = entryPrice || currentLtp;
        const tgt1 = target1Price || (baseEntry * (1 + t1Pct / 100));
        const tgt2 = target2Price || (tgt1 * 1.25);
     ...
     */
    static isTradePriceDistorted(trade, liveLtp, minViableLtp, symbol) {
        ...
        if (trade.isCarriedForward || trade.status === 'CARRIED_FORWARD')
            return false;
        const isBuyer = trade.tradingRole !== 'SELLER';
        const tradeSymbol = (symbol || trade.symbol || '').toUpperCase().trim();
        ...
        if (isBuyer && (trade.entryPrice < floor || (liveLtp > 0 && liveLtp < floor)))
            return true;
        ...
        if (isBuyer && isLateSession && (trade.entryPrice <= lateFloor || (liveLtp > 0 && liveLtp <= lateFloor)))
            return true;
    ...
    static evaluateLifecycleMilestones(params) {
        ...
        // 2. Entry Trigger Check & Exact Time Locking
    ...
    static generateUnifiedTipsPackage(symbol, spotPrice, strikes, masterConfluence, faydaStrategy, allFaydaStrategies, multiLegStrategy, patternBreakout, heroZeroSignals, cprData, marketRegime, pcr, indiaVix, previousSessionTrades = [], technicalIndicators, maxPain, daysToExpiry = 2, activeExpiryDate, upcomingExpiries = [], recentSurges = []) {
        ...
        for (const prev of previousSessionTrades) {
            ...
            let lifecycleDirective = prev.lifecycleDirective || 'HOLD_OR_ACCUMULATE';
        ...
        const primCarryAdvice = primAdvice.carryForwardAdvice;
        const primCarrySuggestion = primAdvice.carryForwardSuggestion;
        const primOngoingProfitBox = primAdvice.ongoingProfitBox;
        ...
        if (slotEntry.sellerPuts.length > 0) {
            ...
            const sellerPutConfluence = ConfluenceEngine.evaluate10IndicatorConfluence(symbol, 'SELL_PUT_SPREAD', spotPrice, soldPutStrike, strikes, pcr, maxPain, technicalIndicators, patternBreakout, cprData, indiaVix);
        ...
        if (slotEntry.sellerCalls.length > 0) {
            ...
            const sellerCallConfluence = ConfluenceEngine.evaluate10IndicatorConfluence(symbol, 'SELL_CALL_SPREAD', spotPrice, soldCallStrike, strikes, pcr, maxPain, technicalIndicators, patternBreakout, cprData, indiaVix);
        ...
        let topSellerNeutralTrade = null;
        if (slotEntry.sellerNeutrals.length > 0) {
            ...
            const condorConfluence = ConfluenceEngine.evaluate10IndicatorConfluence(symbol, 'IRON_CONDOR', spotPrice, atmStrike, strikes, pcr, maxPain, technicalIndicators, patternBreakout, cprData, indiaVix);
        ...
        if (canProcessGamma && topHz && topHz.ltp >= minViableGammaLtp && (sessionInfo.session === 'AFTERNOON_GAMMA_POWER_HOUR' || topHz.gammaScore >= 80)) {
            ...
            const gammaMilestones = ConfluenceEngine.evaluateLifecycleMilestones({
                ...
                strategyTag: '0DTE Gamma Explosion Sniper'
            });
            let gammaStatus = 'ACTIVE';
