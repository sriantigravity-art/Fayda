    ...
    processSnapshot(symbol, spotPrice, spotChange, spotPctChange, strikesRaw, strikeStep, lotSize, defaultRange = 200, dataSource = 'FYERS_LIVE', expiryDates = [], selectedExpiry, exchangeTotalCallOI, exchangeTotalPutOI, indiaVix) {
        ...
        for (const raw of strikesRaw) {
            ...
            if (callSurge.level !== 'NORMAL' && raw.callVolume >= 10000 && Math.abs(strike - atmStrike) <= 350) {
                ...
                if (spotPctChange > 0.05)
                    calibratedCallScore += 6; // Spot trend alignment
                if (callBuyVolPct >= 55)
                    calibratedCallScore += 5; // Aggressive buyer volume dominance
                if (greeks.callIvStatus === 'CHEAP')
                    calibratedCallScore += 4; // Low IV crush risk
                if (callLiq.rating === 'HIGH_LIQUIDITY')
                    calibratedCallScore += 4; // Tight execution spread
                if (callLiq.rating === 'LOW_SLIPPAGE_RISK')
                    calibratedCallScore -= 12; // Penalty for wide spread slippage
            ...
            if (putSurge.level !== 'NORMAL' && raw.putVolume >= 10000 && Math.abs(strike - atmStrike) <= 350) {
                ...
                if (spotPctChange < -0.05)
                    calibratedPutScore += 6; // Spot trend alignment (falling index)
                if (putBuyVolPct >= 55)
                    calibratedPutScore += 5; // Aggressive buyer volume dominance
                if (greeks.putIvStatus === 'CHEAP')
                    calibratedPutScore += 4; // Low IV crush risk
                if (putLiq.rating === 'HIGH_LIQUIDITY')
                    calibratedPutScore += 4; // Tight execution spread
                if (putLiq.rating === 'LOW_SLIPPAGE_RISK')
                    calibratedPutScore -= 12; // Penalty for wide spread slippage
        ...
        const indexState = {
            ...
            heroZeroSignals,
            patternBreakout,
            masterConfluence: ConfluenceEngine.calculateMasterConfluence(symbol, spotPrice, strikesData, pcr, maxPain, straddleRange, daysToExpiry, patternBreakout),
            cprData,
            virginCPRs,
            marketRegime,
