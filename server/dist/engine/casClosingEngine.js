...
export class CasClosingEngine {
     ...
     */
    static getIstDate(nowMs = Date.now()) {
    ...
    static calculateProbableClose(params) {
        ...
        const constituents = CONSTITUENTS_CONFIG.filter(c => {
            if (symbol === 'BANKNIFTY')
                return (c.weightBankNifty || 0) > 0;
            if (symbol === 'FINNIFTY')
                return (c.weightFinNifty || 0) > 0;
            if (symbol === 'SENSEX')
                return (c.weightSensex || 0) > 0;
        ...
        for (const s of relevantStrikes) {
            ...
            const callLtpStatus = spotPrice > k + 1 ? 'ITM' : spotPrice < k - 1 ? 'OTM' : 'ATM';
            const putLtpStatus = spotPrice < k - 1 ? 'ITM' : spotPrice > k + 1 ? 'OTM' : 'ATM';
            let callProjectedStatus = probableClose > k ? 'ITM' : 'OTM';
            let putProjectedStatus = probableClose < k ? 'ITM' : 'OTM';
