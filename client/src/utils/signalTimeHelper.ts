...
export function getUserTradeAdvice(params: {
  ...
  if (isTargetAchieved) {
    return {
      ...
      explanation: `🎯 SYSTEM DIRECTIVE: Target 1 achieved (+${pnlPct}%). Lock profits now or trail stoploss to entry cost.`,
      pnlPoints,
      pnlPct,
  ...
  if (isStoplossHit) {
    return {
      ...
      explanation: `🛑 SYSTEM DIRECTIVE: Stoploss floor breached at ₹${stoplossPrice.toFixed(2)} (${pnlPct}%). Liquidate position to preserve capital.`,
      pnlPoints,
      pnlPct,
      isTargetAchieved: false,
      isStoplossHit: true,
  ...
  if (isExpired) {
    return {
      ...
      explanation: `⏳ SYSTEM ADVISORY: Signal exceeded active ${maxMin}m validity window. Shifted to post-market performance ledger.`,
      pnlPoints,
      pnlPct,
      isTargetAchieved: false,
      isStoplossHit: false,
  ...
  if (targetDistance > 0 && currentDistance / targetDistance >= 0.60) {
    return {
      ...
      explanation: `🚀 SYSTEM DIRECTIVE: In strong profit (+${pnlPct}%). Trail stoploss to breakeven (₹${cleanEntry.toFixed(2)}) for risk-free ride.`,
      pnlPoints,
      pnlPct,
      isTargetAchieved: false,
      isStoplossHit: false,
  ...
  if (elapsedMinutes < 5 || (currentLtp >= cleanEntry * 0.97 && currentLtp <= cleanEntry * 1.03)) {
    return {
      ...
      explanation: `🟢 SYSTEM DIRECTIVE: Prime entry zone (₹${(cleanEntry * 0.98).toFixed(2)}–₹${(cleanEntry * 1.02).toFixed(2)}). Enter position on momentum trigger.`,
      pnlPoints,
      pnlPct,
      isTargetAchieved: false,
      isStoplossHit: false,
  ...
  return {
    ...
    explanation: `⏸️ SYSTEM ADVISORY: Consolidating safely above SL (₹${stoplossPrice.toFixed(2)}). Hold position towards target.`,
    pnlPoints,
    pnlPct,
    isTargetAchieved: false,
    isStoplossHit: false,
