...
export const RightAnalyticsColumn: React.FC = () => {
  ...
  const totalCallOi = pcr.totalCallOi || 1;
  const totalPutOi = pcr.totalPutOi || 1;
    ...
    <div className="flex flex-col space-y-3.5 select-none">
      ...
      <div className="bg-terminal-card border border-terminal-border rounded-xl p-3.5 shadow-subtle space-y-3 font-sans">
        ...
        <div className="grid grid-cols-3 gap-2 text-center font-mono">
          <div className="p-2 rounded-lg bg-terminal-panel/60 border border-terminal-border">
            ...
            <span className="text-sm font-bold text-terminal-text tabular-nums">{pcr.atmPlusMinus5Pcr.toFixed(2)}</span>
          </div>

          <div className="p-2 rounded-lg bg-terminal-panel/60 border border-terminal-border">
            <span className="text-[10px] text-terminal-muted font-sans font-medium uppercase block">Total PCR</span>
            <span className="text-sm font-bold text-terminal-text tabular-nums">{pcr.totalPcr.toFixed(2)}</span>
          </div>

          <div className="p-2 rounded-lg bg-terminal-panel/60 border border-terminal-border">
            <span className="text-[10px] text-terminal-muted font-sans font-medium uppercase block">PCR Shift</span>
            <span className={`text-sm font-bold tabular-nums ${pcr.pcrChange >= 0 ? 'text-bull' : 'text-bear'}`}>
              {pcr.pcrChange > 0 ? '+' : ''}{pcr.pcrChange.toFixed(2)}
            </span>
          </div>
        </div>
      ...
      <div className="bg-terminal-card border border-terminal-border rounded-xl p-3.5 shadow-subtle space-y-3 font-sans">
        ...
        <div className="space-y-2 font-mono text-xs">
          {/* Top Resistance Wall */}
          {resistanceLevels.slice(0, 2).map((res, idx) => (
            <div key={idx} className="p-2 rounded-lg bg-bear/5 border border-bear/20 flex items-center justify-between">
              ...
              <div className="text-right text-[11px] text-terminal-muted">
                <span className="text-bear font-medium">{res.oiFormatted}</span> ({(res.strikePrice - spotPrice).toFixed(0)} pts away)
              </div>
            </div>
          ))}
          ...
          <div className="py-1 px-2.5 rounded-lg bg-terminal-panel border border-terminal-border flex items-center justify-between text-xs">
            ...
            <span className="font-bold text-terminal-text tabular-nums">₹{spotPrice.toLocaleString('en-IN', { minimumFractionDigits: 1 })}</span>
          </div>

          {/* Top Support Wall */}
          {supportLevels.slice(0, 2).map((sup, idx) => (
            <div key={idx} className="p-2 rounded-lg bg-bull/5 border border-bull/20 flex items-center justify-between">
              ...
              <div className="text-right text-[11px] text-terminal-muted">
                <span className="text-bull font-medium">{sup.oiFormatted}</span> ({(spotPrice - sup.strikePrice).toFixed(0)} pts below)
              </div>
            </div>
          ))}
        </div>
        ...
        {straddleRange && (
          <div className="p-2.5 rounded-lg bg-terminal-panel/60 border border-terminal-border space-y-1 font-mono text-xs">
            <div className="flex justify-between text-[11px] text-terminal-muted">
              <span>Lower Bound: <strong className="text-terminal-text">{straddleRange.lowerBound}</strong></span>
              <span>Upper Bound: <strong className="text-terminal-text">{straddleRange.upperBound}</strong></span>
            </div>
            <div className="text-[10px] text-terminal-muted font-sans text-center">
              Expected Expiry Band Width: ±{straddleRange.expectedMove} pts
            </div>
          </div>
        )}
      </div>
